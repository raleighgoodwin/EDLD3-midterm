...
## 2. 
```{r}
plot <- left_join(results_1, results_2, by = "district") 

plot <- plot %>% 
  mutate(missing_1 = map_lgl(gap.x, ~ nrow(.x)==0),
         missing_2 = map_lgl(gap.y, ~ nrow(.x)==0)) %>%
  filter(missing_1 == FALSE & missing_2 == FALSE) %>% 
